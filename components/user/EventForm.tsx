...
const EventForm: React.FC = () => {
  ...
  const actions = (
      <button onClick={() => navigate('/user/dashboard')} className="flex items-center px-3 py-2 text-sm bg-white border border-gray-300 rounded-lg shadow-sm hover:bg-gray-100 transition-colors">
              ...
              return (
                <div
                  ...
                  className={`p-4 border rounded-lg transition-all ${
                    isFull ? 'bg-gray-200 opacity-70 cursor-not-allowed' : 'cursor-pointer hover:border-brand-primary'
                  } ${isSelected ? 'border-brand-primary ring-2 ring-brand-primary' : 'border-gray-300'}`}
                >
                  <div className="flex justify-between items-center">
                    <label htmlFor={option.id} className="font-medium text-brand-dark">{option.label}</label>
                    <span className={`text-sm font-semibold ${isFull ? 'text-red-600' : 'text-gray-600'}`}>
                      {isFull ? 'Full' : `${submissionCount} / ${option.limit} registered`}
                    </span>
                  </div>
                </div>
              );
