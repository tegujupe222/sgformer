...
const ConfirmationPage: React.FC = () => {
  ...
  if (!submissionId)
    return <PageWrapper title="Error">Invalid confirmation.</PageWrapper>;

  const submission = getSubmissionById(submissionId);

  if (!submission)
    return <PageWrapper title="Error">Submission not found.</PageWrapper>;

  const form = getFormById(submission.formId);

  if (!form)
    return <PageWrapper title="Error">Associated form not found.</PageWrapper>;
    ...
    <div className="container mx-auto p-4 sm:p-6 lg:p-8 text-center">
      <div className="bg-white rounded-xl shadow-lg p-8 max-w-2xl mx-auto">
        ...
        <p className="text-gray-600 mb-6">
          ご登録ありがとうございます。イベントの詳細は以下の通りです。
        </p>
        <p className="text-sm text-gray-500 mb-8">
          A confirmation has been simulated as sent to your email at{' '}
